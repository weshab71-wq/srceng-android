...
# Ensure NDK_HOME points to valid directory before any build function runs
export NDK_HOME="${NDK_HOME:-$ROOT_DIR/ndk-binaries}"
...
build()
{
    ...
    if [ ! -f "$NDK_HOME/ndk-build" ]; then
        echo "Error: $NDK_HOME/ndk-build does not exist at $NDK_HOME."
        exit 1
    fi
